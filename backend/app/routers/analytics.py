...
def load_dataframe():
    ...
    for p in CSV_PATHS:
        if pathlib.Path(p).exists():
            logger.info("Analytics: loading CSV from %s", p)
            df = pd.read_csv(p)
            df.columns = [c.strip().lower().replace(" ", "_") for c in df.columns]
            logger.info("Analytics: loaded %d rows, columns: %s", len(df), list(df.columns))
            return df
    raise FileNotFoundError(
        "egypt_home_pricing_30k.csv not found. "
        "Place it at backend/ml/egypt_home_pricing_30k.csv in the repo."
    )
...
def ensure_columns(df):
    ...
    defaults = {
        ...
        "roi_pct":               lambda d: pd.Series(10.0, index=d.index),
        "rental_yield":          lambda d: pd.Series(6.0,  index=d.index),
        "overpriced":            lambda d: (d["price_egp"] > d["price_egp"].median() * 1.15).astype(int),
        "amenity_score":         lambda d: pd.Series(0, index=d.index),
        ...
        "bedrooms":      lambda d: d.get("rooms",        pd.Series(2, index=d.index)),
        "bathrooms":     lambda d: pd.Series(1, index=d.index),
    }
    for col, fn in defaults.items():
        if col not in df.columns:
            try:
                ...
                logger.warning("Could not derive column %s: %s", col, e)
                df[col] = 0
    return df
...
def compute_analytics():
    ...
    try:
        ...
        if 0 in cp.columns and 1 in cp.columns:
            cp = cp.reset_index(); cp.columns = ["area","non_compound","compound"]
            cp = cp.dropna()
        ...
        def safe_prem(mask_a, mask_b):
            a = df[mask_a]["price_egp"].mean()
            b = df[mask_b]["price_egp"].mean()
            return round(float(a/b*100-100), 1) if b and b > 0 and not np.isnan(a) and not np.isnan(b) else 0.0

        metro_prem = safe_prem(df["distance_to_metro_km"]<1,  df["distance_to_metro_km"]>3)
        pool_prem  = safe_prem(df["has_pool"]==1,              df["has_pool"]==0)
        comp_prem  = safe_prem(df["is_compound"]==1,           df["is_compound"]==0)
        ...
        cl_map      = {"price_egp":"Price","area_sqm":"Size","bedrooms":"Beds","bathrooms":"Baths",
                       "floor_number":"Floor","building_age_years":"Age",
                       "distance_to_center_km":"CenterDist","distance_to_metro_km":"MetroDist",
                       "amenity_score":"Amenities","price_per_sqm":"Price/m²"}
        cl_labels   = [cl_map[c] for c in corr_cols]
        ...
        num_cols = [c for c in ["price_egp","area_sqm","price_per_sqm","bedrooms","bathrooms",
                    ...
                    "distance_to_metro_km","rental_yield","roi_pct","amenity_score"]
                    if c in df.columns]
        ...
        studio_y = round(float(df[df["property_type"]=="Studio"]["rental_yield"].mean()),2) \
                   if safe_col("property_type") and "Studio" in df["property_type"].values else 6.5
        ...
        _cache = {
            "kpis": {
                ...
                "avg_bedrooms":      round(float(df["bedrooms"].mean()),1),
            ...
            "metro_hist":    hist("distance_to_metro_km",0,30,30) if safe_col("distance_to_metro_km") else [],
            "age_hist":      hist("building_age_years",0,50,25) if safe_col("building_age_years") else [],
            ...
            "property_types":df["property_type"].value_counts().reset_index().to_dict("records") if safe_col("property_type") else [],
            "conditions":    df["condition"].value_counts().reset_index().to_dict("records") if safe_col("condition") else [],
            "finishing":     df["finishing_type"].value_counts().reset_index().to_dict("records") if safe_col("finishing_type") else [],
            "furnished_dist":df["furnished"].value_counts().reset_index().to_dict("records") if safe_col("furnished") else [],
            "view_dist":     df["view_type"].value_counts().reset_index().to_dict("records") if safe_col("view_type") else [],
            "price_buckets": [
            ...
            "cond_price":      df.groupby("condition")["price_egp"].mean().reset_index().round(0).to_dict("records") if safe_col("condition") else [],
            "fin_price":       df.groupby("finishing_type")["price_egp"].mean().reset_index().round(0).to_dict("records") if safe_col("finishing_type") else [],
            "view_price":      df.groupby("view_type")["price_egp"].mean().reset_index().round(0).to_dict("records") if safe_col("view_type") else [],
            "furnished_price": df.groupby("furnished")["price_egp"].mean().reset_index().round(0).to_dict("records") if safe_col("furnished") else [],
            "bed_stats":       df.groupby("bedrooms").agg(
                               ...
                               ).reset_index().round(2).to_dict("records") if safe_col("bedrooms") else [],
            "floor_price":   df.groupby("floor_number")["price_egp"].mean().reset_index().round(0).to_dict("records") if safe_col("floor_number") else [],
            "age_price":     banded("building_age_years",[-1,1,5,10,20,30,100],["New","1-5yr","5-10yr","10-20yr","20-30yr","30yr+"]) if safe_col("building_age_years") else [],
            "sqm_price":     banded("area_sqm",[0,60,100,150,200,300,900],["<60","60-100","100-150","150-200","200-300","300+"]),
            "metro_price":   banded("distance_to_metro_km",[0,1,2,3,5,10,50],["<1km","1-2km","2-3km","3-5km","5-10km",">10km"]) if safe_col("distance_to_metro_km") else [],
            "center_price":  banded("distance_to_center_km",[0,5,10,15,20,30,100],["<5km","5-10km","10-15km","15-20km","20-30km",">30km"]) if safe_col("distance_to_center_km") else [],
            "cond_fin":      df.groupby(["condition","finishing_type"])["price_egp"].mean().reset_index().round(0).to_dict("records") if all(safe_col(c) for c in ["condition","finishing_type"]) else [],
            "furn_fin":      df.groupby(["furnished","finishing_type"])["price_egp"].mean().reset_index().round(0).to_dict("records") if all(safe_col(c) for c in ["furnished","finishing_type"]) else [],
            "bed_bath":      df.groupby(["bedrooms","bathrooms"])["price_egp"].mean().reset_index().round(0).to_dict("records") if all(safe_col(c) for c in ["bedrooms","bathrooms"]) else [],
            ...
            "scatter_sqm":   df[["area_sqm","price_egp","property_type","area"]].sample(min(600,len(df)),random_state=42).round(1).to_dict("records") if all(safe_col(c) for c in ["area_sqm","price_egp","property_type","area"]) else [],
            "scatter_metro": df[["distance_to_metro_km","price_egp","area","property_type"]].sample(min(500,len(df)),random_state=99).round(2).to_dict("records") if all(safe_col(c) for c in ["distance_to_metro_km","price_egp","area","property_type"]) else [],
            "scatter_age":   df[["building_age_years","price_egp","property_type"]].sample(min(400,len(df)),random_state=77).round(1).to_dict("records") if all(safe_col(c) for c in ["building_age_years","price_egp","property_type"]) else [],
